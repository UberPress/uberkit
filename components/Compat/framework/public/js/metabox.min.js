(function($) {
	...
	$(document).on('click', '.vp-wpa-group-title', function(e){
		...
		$( 'input:focus ').blur(); // fix for annoying scrolling
		...
		if( control.hasClass( 'vp-hide' ) ) {
			...
			if( siblings.exists() ) {
				...
				siblings.each(function( i, el ) {
					...
					$( this ).find( '.vp-controls' ).first().slideUp( 'fast', function() {
						...
						$(this).addClass( 'vp-hide' ).slideDown( 0, function() {
							...
							if( i == siblings.length - 1 ) {
								...
								container.promise().done( function() {
									...
									control.slideUp( 0, function() {
										...
										$(this).removeClass( 'vp-hide' ).slideDown( 'fast' );
			...
			control.slideUp('fast', function() {
				$(this).addClass('vp-hide').slideDown(0);
			});
	...
	function vp_init_fields($elements)
	{
		$elements.each(function(){
			if($(this).parents('.tocopy').length <= 0)
			{
				...
				var id         = $(this).attr('id'),
					name       = $(this).attr('id'),
					rules      = $(this).data('vp-validation'),
					bind       = $(this).data('vp-bind'),
					items_bind = $(this).data('vp-items-bind'),
					dep        = $(this).data('vp-dependency'),
	...
	function vp_init_groups($elements)
	{
		$elements.each(function(){
			...
			{
				var dep  = $(this).data('vp-dependency'),
					type = $(this).getDatas().type,
					id   = $(this).attr('id');
				if(typeof dep !== 'undefined' && dep !== false)
				{
	...
	/**
	 * Metabox dynamic repeatable group titles
	 * 
	 * @author	Nik Schulze
	 *
	 */
	(function() {
		var updateTitle = function($group, title)
		{
			var $title = $group.find('.vp-wpa-group-title');

			var $titleText = $title.contents().filter(function()
			{
				return this.nodeType === 3; // Node.TEXT_NODE
			});

			$titleText.replaceWith(' ' + title);
		}

		$('.vp-wpa-loop[data-title]').each(function(ndx, wrapper)
		{
			var $wrapper = $(wrapper);
			var $groups = $wrapper.find('.vp-wpa-group');
			var fieldName = $wrapper.data('title');

			$groups.each(function(ndx, group)
			{
				var $group = $(group);
				var $titleField = $group.find('input[type="text"][name$="[' + fieldName + ']"]');

				var title = $titleField.val();
				if(title.length)
					updateTitle($group, title);
			});
		});

		$(document).on('keyup', '.vp-wpa-loop[data-title] input.vp-input[type="text"]', function()
		{
			var $field = $(this);
			var $group = $field.parents('.vp-wpa-group');
			var $loop  = $group.parent();

			var match = $field.attr('name').match(/\[(\w+)\]$/);
			if(match && match[1] == $loop.data('title'))
				updateTitle($group, $field.val());
		});
	})();
