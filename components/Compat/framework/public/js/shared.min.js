...
if ( vp_wp.use_new_media_upload ) {
	...
	vp.upload_callback = function(e) {
		...
		var $buttons			= $container.children('.input').children('.buttons');
		var $preview			= $container.find('img');
		var $previewContainer	= $container.find('.image');
		...
		wp.media.editor.send.attachment = function(props, attachment) {
			...
			if(attachment.type === 'image') {
				...
				$previewContainer.removeClass('icon');
			} else {
				$preview.attr('src', attachment.icon);
				$previewContainer.addClass('icon');
			}
				
			$buttons.addClass( 'uk-hide' ).removeClass( 'uk-show' );
			$previewContainer.addClass( 'uk-show' ).removeClass( 'uk-hide' );
...
vp.remove_upload_callback = function(e) {
	...
	$input.val('');
	$preview.attr('src', '');
	$buttons.addClass( 'uk-show' ).removeClass( 'uk-hide' );
	$previewContainer.addClass( 'uk-hide' ).removeClass( 'uk-show' );
	
};

jQuery(document).on('click', '.uk-js-upload', vp.upload_callback);
jQuery(document).on('click', '.uk-js-remove-upload', vp.remove_upload_callback);
...
vp.init_socicon_chooser = function($elements)
{
	if (jQuery.fn.select2)
	{
		...
		var format = function vp_socicon_chooser_format(icon){
			return '<span class="socicon ' + icon.id + '"></span> ' + icon.text;
		};
		$elements.select2({
			formatResult: format,
			formatSelection: format,
		...
		// Code for select2 v4+ (experimental stage)
//		function formatState (icon) {
//		  if (!icon.id) { return icon.text; }
//		  var i = jQuery('<span class="socicon ' + icon.id + '"></span> ' + icon.text);
//		  return i;
//		};
//		
//		$elements.select2({
